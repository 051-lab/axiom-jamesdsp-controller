...
// Added for Async EQ Updates (Double Buffering)
extern void* MultimodalEqualizerCompute(JamesDSPLib *jdsp, int interpolationMode, int operatingMode, double *freqAx, double *gaindB, unsigned int targetBlockSize);
extern void* MultimodalEqualizerApply(JamesDSPLib *jdsp, int operatingMode, void* newConv);
extern void MultimodalEqualizerFreeConv(void *conv);
