...
{
    // HEURISTIC: Fix 0.0 offset passed by script
    if (*start < 1.0f) *start = 20000.0f;
...
static float NSEEL_CGEN_CALL _eel_iirBandSplitterInit(void *opaque, INT_PTR num_param, float **parms)
{
    ...
    // SAFETY: Default to 48kHz if unitialized to prevent crash (JamesDSP Windows Port)
    if (fs < 1.0f) fs = 48000.0f;
